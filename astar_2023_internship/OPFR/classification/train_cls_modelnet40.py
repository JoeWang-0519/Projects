...
def parse_args():
    ...
    parser.add_argument('--estimate_normals', action='store_true', default=False, help='use estimate normals')
...
def main(args):
    ...
    # create pfh directory
    pfh_dir = Path(os.path.join(args.data_dir, 'modelnet40_pfh_estimate_v2'))
    pfh_dir.mkdir(exist_ok=True)
    # create estimate normal directory
    normal_dir = Path(os.path.join(args.data_dir, 'modelnet40_normal_v2'))
    ...
    PFH_PATH = os.path.join(args.data_dir, 'modelnet40_pfh_estimate_v2')
    NORMAL_PATH = os.path.join(args.data_dir, 'modelnet40_normal_v2')

    TRAIN_DATASET = ModelNetDataLoader(root=DATA_PATH, pfh_source=PFH_PATH, normal_source=NORMAL_PATH, args=args, split='train', process_data=False)
    TEST_DATASET = ModelNetDataLoader(root=DATA_PATH, pfh_source=PFH_PATH, normal_source=NORMAL_PATH, args=args, split='test', process_data=False)
