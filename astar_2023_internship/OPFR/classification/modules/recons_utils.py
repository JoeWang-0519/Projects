...
def cal_normal(group_xyz, pfh=False, normal_shape=4, random_inv=False, is_group=False):
    """
    Calculate Normal Vector (Unit Form + First Term Positive)

    :param group_xyz: [B, N, K=3, 3] / [B, N, G, K=3, 3] / [B, N, k2, k3, K=3, 3]
    :param random_inv:
    ...
    """
    if pfh:
        edge_vec1 = group_xyz[..., 1, :] - group_xyz[..., 0, :]  # [B, N, k2, k3, 3]
        edge_vec2 = group_xyz[..., 3, :] - group_xyz[..., 0, :]  # [B, N, k2, k3, 3]
    else:
        edge_vec1 = group_xyz[..., 1, :] - group_xyz[..., 0, :]  # [B, N, k2, k3, 3]
        edge_vec2 = group_xyz[..., 2, :] - group_xyz[..., 0, :]  # [B, N, k2, k3, 3]

    nor = torch.cross(edge_vec1, edge_vec2, dim=-1)
    unit_nor = nor / torch.norm(nor, dim=-1, keepdim=True)  # [B, N, 3] / [B, N, G, 3] / [B， N, k2, k3, 3]
    if not is_group:
        # the mask is the same only for each data point
        pos_mask = (unit_nor[..., 0] > 0).float() * 2. - 1.  # keep x_n positive
    else:
        # the mask is the same for the same neighborhood
    ...
    if random_inv:
        ...
        if normal_shape == 3:
            ...
            unit_nor = unit_nor * random_mask.unsqueeze(-1).unsqueeze(-1)
...
def cal_center(group_xyz):
    ...
    :param group_xyz: [B, N, K, 3] / [B, N, G, K, 3] / [B, N, k2, k3, K, 3]; K >= 3
    :return: [B, N, 3] / [B, N, G, 3] / [B, N, k2, k3, 3]
    """
...
def cal_const(normal, center, is_normalize=True):
    ...
    :param is_normalize:
    :param normal: [B, N, 3] / [B, N, G, 3] / [B, N, k2, k3, 3]
    :param center: [B, N, 3] / [B, N, G, 3] / [B, N, k2, k3, 3]
    :return: [B, N, 1] / [B, N, G, 1] / [B, N, k2, k3, 1]
    """
...
def cal_angle(xyz, normal):
    '''
    :param xyz: [B, N, k2, k3, 4, 3]
    :param normal: [B, N, k2, k3, 3]
    :return: [B, N, k2, k3, 3]
    '''
    # [B, N, k2, k3, 3]
    vec1, vec2, vec3 = xyz[..., 1, :], xyz[..., 2, :], xyz[..., 3, :]
    unit_vec1 = vec1 / torch.norm(vec1, dim=-1, keepdim=True)
    unit_vec2 = vec1 / torch.norm(vec1, dim=-1, keepdim=True)
    unit_vec3 = vec1 / torch.norm(vec1, dim=-1, keepdim=True)
    # [B, N, k2, k3, 1]
    theta1 = torch.sum(torch.mul(unit_vec1, unit_vec2), dim=-1, keepdim=True)
    theta2 = torch.sum(torch.mul(unit_vec2, unit_vec3), dim=-1, keepdim=True)
    theta3 = torch.sum(torch.mul(unit_vec1, unit_vec3), dim=-1, keepdim=True)
    theta4 = torch.sum(torch.mul(normal, unit_vec2), dim=-1, keepdim=True)
    # [B, N, k2, k3, 4]
    return torch.cat([theta1, theta2, theta3, theta4], dim=-1)
...
def check_nan_umb(normal, center, pos=None):
    ...
    # re-write the NaN entry with first non-NaN entry
...
def check_nan_umb_v2(normal, center, pos=None):
    """
    Check & Remove NaN in normal tensor

    :param pos: [B, N, k2, k3, 1]
    :param center: [B, N, k2, k3, 3]
    :param normal: [B, N, k2, k3, 3]
    :return:
    """
    B, N, k2, k3, _ = normal.shape
    # [B, N, k2, k3]
    mask = torch.sum(torch.isnan(normal), dim=-1) > 0
    # [B, N, k2]
    mask_first = torch.argmax((~mask).int(), dim=-1)
    b_idx = torch.arange(B).view([B, 1, 1]).repeat([1, N, k2])
    n_idx = torch.arange(N).view([1, N, 1]).repeat([B, 1, k2])
    k2_idx = torch.arange(k2).view([1, 1, k2]).repeat([B, N, 1])

    # [B, N, k2, k3, 3]
    normal_first = normal[b_idx, n_idx, k2_idx, None, mask_first].repeat([1, 1, 1, k3, 1])
    normal[mask] = normal_first[mask]
    center_first = center[b_idx, n_idx, k2_idx, None, mask_first].repeat([1, 1, 1, k3, 1])
    center[mask] = center_first[mask]

    if pos is not None:
        pos_first = pos[b_idx, n_idx, k2_idx, None, mask_first].repeat([1, 1, 1, k3, 1])
        pos[mask] = pos_first[mask]
        return normal, center, pos
...
def check_nan_pfh(normal, center, angle, pos=None):
    """
    Check & Remove NaN in normal tensor
    ...
    center_first = center[b_idx, n_idx, None, mask_first].repeat([1, 1, G, 1])
    center[mask] = center_first[mask]
    angle_first = angle[b_idx, n_idx, None, mask_first].repeat([1, 1, G, 1])
    angle[mask] = angle_first[mask]

    if pos is not None:
        pos_first = pos[b_idx, n_idx, None, mask_first].repeat([1, 1, G, 1])
        pos[mask] = pos_first[mask]
        return normal, center, angle, pos
    return normal, angle, center


def check_nan_pfh_v2(normal, center, angle, pos=None):
    """
    Check & Remove NaN in normal tensor

    :param pos: [B, N, k2, k3, 1]
    :param center: [B, N, k2, k3, 3]
    :param angle: [B, N, k2, k3, 4]
    :param normal: [B, N, k2, k3, 3]
    :return:
    """
    B, N, k2, k3, _ = normal.shape
    ...
    center_first = center[b_idx, n_idx, k2_idx, None, mask_first].repeat([1, 1, 1, k3, 1])
    center[mask] = center_first[mask]
    angle_first = angle[b_idx, n_idx, k2_idx, None, mask_first].repeat([1, 1, 1, k3, 1])
    angle[mask] = angle_first[mask]
