...
def parse_args():
    ...
    parser.add_argument('--group_size3', type=int, default=8, help='Size of umbrella groups [default: 8]')
...
def main(args):
    ...
    '''CREATE DIR'''
    normal_dir = Path(os.path.join(args.data_dir, 'ScanObjectNN', 'normal_50ngb'))
    normal_dir.mkdir(exist_ok=True)

    pfh_dir = Path(os.path.join(args.data_dir, 'ScanObjectNN', 'pfh_20ngb_normal_50ngb'))
    ...
    # need to be modified if want another dataset
    '''DATA LOADING'''
    ...
    TRAIN_DATASET = ScanObjectNNDataLoader(root=DATA_PATH, use_normals = args.use_normals, use_pfh = args.use_pfh, split='training')
    TEST_DATASET = ScanObjectNNDataLoader(root=DATA_PATH, use_normals = args.use_normals, use_pfh = args.use_pfh, split='test')
