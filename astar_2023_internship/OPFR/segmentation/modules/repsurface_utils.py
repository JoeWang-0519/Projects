...
from modules.recons_utils import cal_const, cal_normal, cal_center, check_nan_umb, cal_angle, check_nan_pfh
...
def group_by_pfh(xyz, new_xyz, offset, new_offset, k=9):
    """
        Group a set of points into umbrella surfaces
...
class SurfaceAbstractionCD(nn.Module):
    ...
    def forward(self, pos_nor_feat_off):
        center, normal, feature, offset = pos_nor_feat_off  # [N, 3], [N, 10/14], [N, C], [B]
        ...
        # new_center: sampled points position data, [M, 3]
        # new_normal: sampled normal feature data, [M, 3/10/14]
        # new_feature: sampled feature, [M, nsample, 3+3/10/14+C]
        new_feature = new_feature.transpose(1, 2).contiguous()  # [M, 3+C, nsample]
...
class PFHSurfaceConstructor(nn.Module):
    ...
    """
    def __init__(self, k, in_channel, out_channel, random_inv=True, sort='pfh'):
    ...
    def forward(self, center, offset):
        ...
        group_xyz = self.sort_func(center, center, offset, offset, k=self.k)  # [N, K-1, 4 (points), 3 (coord.)]

        # normal
        group_normal = cal_normal(group_xyz, offset, random_inv=self.random_inv, is_group=True, PFH=True) # [N, K-1, 3]
        # coordinate
        group_center = cal_center(group_xyz)    # [N, K-1, 3]
        # polar
        group_polar = xyz2sphere(group_center) # [N, K-1, 3]
        # surface position
        group_pos = cal_const(group_normal, group_center)   # [N, K-1, 1]
        # angle (PFH)
        group_angle = cal_angle(group_xyz, group_normal) # [N, K-1, 4]

        group_normal, group_center, group_pos, group_angle = check_nan_pfh(group_normal, group_center, group_pos, group_angle)
        new_feature = torch.cat([group_polar, group_normal, group_pos, group_center, group_angle], dim=-1)  # P+N+SP+C+A: 14
        new_feature = new_feature.transpose(1, 2).contiguous()  # [N, 14, G]
