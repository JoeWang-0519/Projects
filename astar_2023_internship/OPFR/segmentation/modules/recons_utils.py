...
def cal_normal(group_xyz, offset, random_inv=False, is_group=False, PFH=False):
    """
    Calculate Normal Vector (Unit Form + First Term Positive)

    :param group_xyz: [N, G, 3, 3] / [N, G, 4, 3]

    return:
    [N, G, 3]
    """
    if not PFH:
        edge_vec1 = group_xyz[..., 1, :] - group_xyz[..., 0, :]  # [N, G, 3]
        edge_vec2 = group_xyz[..., 2, :] - group_xyz[..., 0, :]  # [N, G, 3]
    else:
        edge_vec1 = group_xyz[..., 1, :] - group_xyz[..., 0, :]  # [N, G, 3]
        edge_vec2 = group_xyz[..., 3, :] - group_xyz[..., 0, :]  # [N, G, 3]

    nor = torch.cross(edge_vec1, edge_vec2, dim=-1)
    unit_nor = nor / torch.norm(nor, dim=-1, keepdim=True)  # [N, G, 3]
    if not is_group:
...
def check_nan_pfh(normal, center, pos, angle):
    """
    Check & Remove NaN in normal tensor
    ...
    center_first = center[torch.arange(N), None, mask_first].repeat([1, G, 1])
    center[mask] = center_first[mask]
    angle_first = angle[torch.arange(N), None, mask_first].repeat([1, G, 1])
    angle[mask] = angle_first[mask]

    if pos is not None:
        pos_first = pos[torch.arange(N), None, mask_first].repeat([1, G, 1])
        pos[mask] = pos_first[mask]
        return normal, center, pos, angle
